...
Un langage de programmation interprété sur lequel j'ai travaillé pendant mes études (avec un mode "compilé" expérimental, plus de détails [ici]()). Aujourd'hui, je travaille principalement sur ce mode compilé susmentionné et sur la robustesse globale du programme.  

## Configuration

L'interpréteur peut être construit en exécutant le makefile fourni avec `make` (plus de détails concernant les autres commandes du makefile se trouvent [ici]()). De plus, un programme pour vérifier les lexèmes est également disponible, celui-ci peut être construit en exécutant make avec la variable `LEXEME` définie (par exemple `make LEXEME=1`).  
...
Les détails concernant la syntaxe peuvent être trouvés (en français) dans [compte_rendu.pdf]().  